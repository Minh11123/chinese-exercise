$(document).ready(function(){	
	...
	function ajaxGet(page){
		...
		$.ajax({
			type: "GET",		
			url: "http://localhost:8080/webTOCFL/api/client/bai-nghe/all" + "?page=" + page + "&doKho=" + doKho + "&part=" + part,
			success: function(result){
				if(result.totalElements > 0){
					$("#pTag").removeClass().addClass("hidden");
					$.each(result.content, function(i, baiNghe){
//						var li = '<li><a href="/webTOCFL/listening/part-'+baiNghe.part+'/'+baiNghe.id+'">'+baiNghe.tenBaiNghe+'</a></li>';
						var html = '<div class="span9">'
							+ '<div class="span3">' 
							+    '<img class="imageExam" src="/webTOCFL/file/images/baiNgheId='+baiNghe.id+'.png" />"'
							+' </div>'
							+ '<div class="span1"></div>'
							+ '<div class="span5"> '
							+    '<h4 class="content-heading" id="namebaithithu">'+ baiNghe.tenBaiNghe + '</h4>'
							+    '<a class="btn btn-primary" href="/webTOCFL/listening/part-'+baiNghe.part+'/'+baiNghe.id+'"> Chi tiết</button>'
							+  '</div>'
							+ '</div>'
